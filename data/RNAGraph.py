...
# tensor_list[i]对应每个序列的tensor
    ...
    def _constructGraph(self, seq, csr_matrix, tensorline):
        ...
        grh.ndata['feat'] = torch.zeros((grh.number_of_nodes(), 29))

        #节点表征
        for i in range(len(seq)):
            feature = torch.cat((d[seq_upper[i]].double(), tensorline[i].reshape([1, -1])), dim=1)
            grh.ndata['feat'][i] = feature
...
class RNADataset(torch.utils.data.Dataset):
    def __init__(self, name, config, fold_algo, window_size=501):
        ...
        with open(data_dir + name + fold_algo + '_29.pkl', "rb") as f:
            f = pickle.load(f)
            self.train = f[0]
            self.val = f[1]
            self.test = f[2]
