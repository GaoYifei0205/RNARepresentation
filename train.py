...
"""
    loss record
"""
def loss_record(DATASET_NAME, print_str, epoch, config):
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs, config):
    ...
    try:
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                epoch_val_loss, epoch_val_acc, epoch_val_auc = evaluate_network(model, device, val_loader, epoch)
                _, epoch_test_acc, epoch_test_auc = evaluate_network(model, device, test_loader, epoch)
                ...
                loss_record(DATASET_NAME, print_list[-1], epoch, config)
                ...
                if last_val_loss > epoch_val_loss:
                    ...
                    if early_stop_count == 10:
                        print("early stop because the val loss doesn't decrease for 10 epochs")
                        break
    ...
    if results_print_str is not None:
        loss_record(DATASET_NAME, results_print_str, epoch, config)
        save_model(DATASET_NAME, model, epoch, config)
    else:
        epoch = 0
        model = load_model(DATASET_NAME, MODEL_NAME, net_params, config, config['best_epoch'])
    ...
    if config['motif'] == "True":
        ...
        model = load_model(DATASET_NAME, MODEL_NAME, net_params, config, config['best_epoch'])
        model = model.to(device)
        detect_motifs(model, train_loader, device, output_dir='motifs/' + DATASET_NAME)

    _, test_acc, test_auc = evaluate_network(model, device, test_loader, epoch)
    _, train_acc, train_auc = evaluate_network(model, device, train_loader, epoch)
...
def main():
    ...
    parser.add_argument('--config', default='configs/RNAgraph_graph_classification_GCN_in_vivo_100k.json',
                        help="Please give a config.json file with training/model/data/param details")
    parser.add_argument('--gpu_id', help="Please give a value for gpu id")
    parser.add_argument('--model', default='GCN', help="Please give a value for model name")
    parser.add_argument('--dataset', default='ALKBH5_Baltz2012', help="Please give a value for dataset name")
    parser.add_argument('--out_dir', help="Please give a value for out_dir")
    parser.add_argument('--seed', default=39, help="Please give a value for seed")
